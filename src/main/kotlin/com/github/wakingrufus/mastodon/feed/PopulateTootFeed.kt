...
fun populateTootFeed(feed: ObservableList<Status>,
    ...
    FeedWorker.logger.debug("fetching Home feed")
    try {
        val statusPageable = fetcher.invoke(Range()).execute()
        val statuses = statusPageable.part
        FeedWorker.logger.info(statuses.size.toString() + " statuses found")
        feed += statuses

    } catch (e: Mastodon4jRequestException) {
        FeedWorker.logger.error("Error fetching feed: " + e.localizedMessage, e)
    }
